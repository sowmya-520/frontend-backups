...
class get_products_bycategory_view(APIView):
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response({ 'categories': "category doesnot exists"}, status=status.HTTP_400_BAD_REQUEST)


class createInvoiceView(APIView):
    permission_classes = [IsAuthenticated]
    def calculate_invoice_amount(self, invoice):
        total_amount = 0
        for product in invoice.products.all():
            # Iterate through the rental options for the product
            for rental_option in product.rentaloptions:
                if rental_option['tenure'] == invoice.tenure:
                    total_amount += int(rental_option['ratepermonth'])
                    break  # Stop searching once the matching tenure is found
    ...
    def post(self, request):
            ...
            if serializer.is_valid():
                serializer.save()
                invoice = get_object_or_404(Invoice, pk=serializer.data['id'])
                invoice.amount = self.calculate_invoice_amount(invoice)
                invoice.save()
